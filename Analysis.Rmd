...
getwd()
write_xlsx(missingid, "/Users/taylorbuck/Desktop/missingid.xlsx")

missingid2 <- read_xlsx("missingid.xlsx")
str(missingid2)
```
...
recdf <- activealloc %>%
  ...
  group_by(cleanname) %>%
  arrange(date) %>%
  mutate(cumpos = accumulate(newpositive, `+`))


recalloc <- activealloc %>%
  mutate(date = as.Date(date)) %>%
  group_by(cleanname) %>%
  mutate(reccases = map_dbl(date, ~sum(newpositive[(date <= . + 14) & date >= .])))

  
  mutate(reccases = for (i in 1:nrow(activealloc)) {
    
  })

for (i in 1:nrow(activealloc)) {
  #getting number of new cases
  x <- activealloc$newpositive[i]
  #add 14 days to the date
  d <- activealloc$date + 14
  activealloc$rec <- sum(x)
}
  
  mutate(reccases = map_dbl(date, ~sum(newpositive[(date <= . + 14) & date >= .])))
  
  
  mutate(reccases = rollsum(x = newpositive, 28, align = "right", fill = NA))
  
  mutate(reccases = map_dbl(date, ~sum(newpositive[(date >= . - 14)])))
```
